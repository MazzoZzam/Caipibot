...
from ..algorithms.linked_list import lista
...
pedido = Table (
    ...
    Column('id_cliente', Integer, ForeignKey(cliente.c.id_cliente, ondelete='CASCADE'), nullable=False),
...
item_pedido = Table (
    ...
    Column('id_pedido', Integer, ForeignKey('pedido.id_pedido', ondelete='CASCADE'), nullable=False)
)
...
def inserir_pedido(id_cliente, lista_itens):
    ...
    for id_item in lista_itens:
        ...
        name_select = select(cardapio.c.nome_item).where(cardapio.c.id_item == id_item)
        name_result = conn.execute(name_select).fetchone()
        itens_name.append(name_result[0])
    ...
    client_select = select(cliente.c.nome_cliente, 
                           cliente.c.telefone,
                           cliente.c.endereco).where(cliente.c.id_cliente == id_cliente)
    ...
    lista.novo_pedido(int(id_new_order), 
                      client_result[0],
                      client_result[1],
                      client_result[2],
                      data_result[0],
                      itens_name,
                      float(price_result[0]))
...
def inserir_pedido_lista():
    ...
    for order in orders_result:
        ...
        lista.novo_pedido(int(order[0]), order[1], order[2], order[3], order[4], itens, total)
...
def ver_aprovados():
    select_orders = select(pedido.c.id_pedido,
                           ...
                           cardapio.c.preco,
                           pedido.c.estado_pedido).select_from(
